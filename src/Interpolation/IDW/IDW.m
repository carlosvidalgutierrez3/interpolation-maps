...
for i=1:length(cat)           % for each cell of the grid:
   if cat(i,3)==0             % only with cells equal 0
      divider = 0;
      % Find BS's in a 3 degree square area centered on the point
      n = find(BS(:,2)<(cat(i,2)+1.5) & BS(:,2)>(cat(i,2)-1.5) & BS(:,1)>(cat(i,1)-1.5) & BS(:,1)<(cat(i,1)+1.5));  
      
      
      d2 = zeros(length(n),1);  % array of squared distances to BS nearby (square)
        
      if ~isempty(n)            % only if there is at least 1 BS inside 
          k = 1;
          for j=1:length(n)     % for list of BS's nearby
              d2(j) =  ((cat(i,1)-BS(n(j),1))^2 + (cat(i,2)-BS(n(j),2))^2)^2;    % Square distance to each BS
%               if d2(j)<0.5                                      % Find BS's in a 1 degree radius
%                   d2_2(k) = d2(j);  % array of squared distances to BS nearby (radius)
%                   divider = divider + 1/d2_2(k);
%                   k = k+1;
%               end
             divider = divider + 1/d2(j);
          end
